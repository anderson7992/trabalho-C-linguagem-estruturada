...
int main()
{
    ...
    int hours, min;
    float vel, tempo, dist, litros;

    printf("Informe o tempo da viagem em horas, e em seguida, minutos: ");
    scanf("%d",&hours);
    scanf("%d",&min);
    ...
    printf("\nDistância percorrida: %.3f Km ou %d Km e %.0f metros", dist, (int)dist, (dist - (int)dist) * 1000);
    printf ("\nTempo da viagem: %d horas e %d minutos : total em horas: %f", hours, min, min / 60.0 + hours);
    printf ("\nVelocidade média: %.1f Km/h", vel);
    printf("\nLitros gastos na viagem: %.3f litros ou %d litros e %.0f ml", litros,(int)litros, (litros - (int)litros) * 1000);
